...
public class Bonus {
    ...
    public static void main(String[] args) {
        ...
        boolean[] explosionActive = new boolean[8];
        ...
        while (true) {
            ...
            if (gameMode==0) { // Menu
                ...
                for (int j = 0; j < 8; j++) {
                    if (explosionActive[j]) {
                        explosionTimer[j]++;
                        if (explosionTimer[j] < 10) {
                            StdDraw.picture(enemyX[j], enemyY[j], "assets/explosionSmall.png", 80, 80);
                        } else if (explosionTimer[j] < 20) {
                            StdDraw.picture(enemyX[j], enemyY[j], "assets/explosionBig.png", 100, 100);
                        } else {
                            explosionActive[j] = false;
                        }
                    }
                }
                // SETTING SCORE
